  ...
  <table class="sessions table is-fullwidth">
    <thead>
      <tr>
        <th>会话(Session Id)</th>
        <th>创建(Created at)</th>
        <th>最后接入(Last accessed at)</th>
        <th>终止(Expiry)</th>
        <th>闲置时长(Max. inactive)<br>(interval)
        </th>
        <th>属性(Attributes)</th>
        <th>
          <sba-confirm-button class="button"
            ...
            <span v-if="deletingAll === 'deleted'">已删除Deleted</span>
            <span v-else-if="deletingAll === 'failed'">失败Failed</span>
            <span v-else><font-awesome-icon icon="trash"/>&nbsp;删除Delete</span>
          </sba-confirm-button>
        </th>
      </tr>
    </thead>
